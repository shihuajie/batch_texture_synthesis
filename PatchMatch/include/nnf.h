		...
		bool check() {
			...
			for (int i = 0; i < size; ++i) {
                ...
                if (!isWithin(target, patches[i]))
                {
                    patches[i].sx = 1;
                    patches[i].sy = 1;
                    patches[i].angleIndex = ANGLESTEP; // added by huajie 2015-9-26
                }
...
#pragma omp parallel for collapse(2)
	...
	public:
		...
		inline bool tryPatch(int y, int x, const Patch &nPatch, float curOcc = -1.0f, float newOcc = -1.0f) {
            Patch newPatch = nPatch;
            ...
            /////////////////////////////////////////////////
			typedef Vec<Scalar, 6> DataType;
			const int width = SourcePatch::width();
            const Scalar invArea = 1.0 / (width * width);
			Scalar d1, d2, trgPatchSum = 0, srcPatchSum = 0, distPatchSum=0;
			const SourcePatch srcPatch(y, x);
            DataType diff, cur_diff,new_diff, trgPixel, srcPixel;
            Scalar curPatchDist = 0, newPatchDist=0, alpha;
            alpha = weightIndex;
            int curPatchAngle = curPatch.angleIndex;
            int newPatchAngle = newPatch.angleIndex;
            int cur_sx = curPatch.sx;
            int cur_sy = curPatch.sy;
            int new_sx = newPatch.sx;
            int new_sy = newPatch.sy;
			...
			for (typename SourcePatch::IndexIterator it = srcPatch.begin(); it; ++it) {
                ...
                float cosA = pm::BasicProp.COS[curPatchAngle];
                float sinA = pm::BasicProp.SIN[curPatchAngle];
                float px = trgPixel[4];
                float py = trgPixel[5];
                ...
                float norm_v1 = sqrt(tx*tx + ty*ty);
                float norm_v2 = sqrt(srcPixel[4]*srcPixel[4] + srcPixel[5]*srcPixel[5]);
                d2 = norm_v1*norm_v2 - abs(srcPixel[4]*tx + srcPixel[5]*ty);
                
                curPatchDist += alpha*d1/255.0 + (1-alpha)*d2;
            ...
            DistValType  newDist;
             ...
             for (typename SourcePatch::IndexIterator it = srcPatch.begin(); it; ++it) {
                ...
                float cosA = pm::BasicProp.COS[newPatchAngle];
                float sinA = pm::BasicProp.SIN[newPatchAngle];
                float px = trgPixel[4];
                float py = trgPixel[5];
                float tx = cosA * px + sinA * py;
                float ty = -sinA * px + cosA * py;
                ...
                newPatchDist += alpha*d1/255.0 + (1-alpha)*d2;
		...
		inline bool propagation(int y, int x, int dy, int dx, bool checkOcc = false) {
            ...
            typedef Vec<Scalar, 6> DataType;
            DataType srcPixel = source->at<DataType>(y, x);
            
			// dy patch
			if (py >= 0 && py < height) {
				deltaPatch(get(py, x), delta, dy, 0);
                
                 // compute the intersection angle of current pixel to its neighbor
                DataType srcPixel_neighbor = source->at<DataType>(py, x);
				float dotP = srcPixel_neighbor[4]*srcPixel[4]+srcPixel_neighbor[5]*srcPixel[5];
				float m1 = sqrt(srcPixel_neighbor[4]*srcPixel_neighbor[4]+srcPixel_neighbor[5]*srcPixel_neighbor[5]);
				float m2 = sqrt(srcPixel[4]*srcPixel[4]+srcPixel[5]*srcPixel[5]);
				float m12 = m1*m2;
				if(m12 < 1.0e-4)
					m12 = 1.0;
				dotP = dotP / m12;
				...
				double delta_angle = acos(fabs( dotP ));
                double angle_cur = MAX_ANGLE / ANGLESTEP * (delta.angleIndex - ANGLESTEP);
                int angle1 = (angle_cur + delta_angle) /  MAX_ANGLE * ANGLESTEP + ANGLESTEP;
                while(angle1 > 2*ANGLESTEP)
                    angle1 = angle1 - 2*ANGLESTEP;
                int angle2 = (angle_cur - delta_angle) /  MAX_ANGLE * ANGLESTEP + ANGLESTEP;
                while(angle2 < 0)
                    angle2 = angle2 + 2*ANGLESTEP;
                
				delta.angleIndex = angle1;                   
			...
			if (px >= 0 && px < width) {
                ...
                DataType srcPixel_neighbor = source->at<DataType>(y, px);
				float dotP = srcPixel_neighbor[4]*srcPixel[4]+srcPixel_neighbor[5]*srcPixel[5];
				float m1 = sqrt(srcPixel_neighbor[4]*srcPixel_neighbor[4]+srcPixel_neighbor[5]*srcPixel_neighbor[5]);
				float m2 = sqrt(srcPixel[4]*srcPixel[4]+srcPixel[5]*srcPixel[5]);
				float m12 = m1*m2;
