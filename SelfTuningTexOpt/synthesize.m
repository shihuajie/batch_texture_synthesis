...
%% add by huajie 2015-9-9
        last = options.nnf_channels(end)
        cur_im(:, :, 8:last) = org_Pyr{k,1}(:, :, 8:last);
 %% end 
        %% The EM iterations
...
%              sprintf('src_mid_im%02d.png', k-1) ]);
    ...
    imwrite(trg_mid_im, [out_directory filesep ...
     ...
     [nnf_angle_im, nnf_flip_im] = drawAngleArrow(cur_nnf(:,:,3:5));
     imwrite(nnf_angle_im, [out_directory filesep ...
              sprintf('nnf_angle_im%02d.png', j) ]);    
     imwrite(nnf_flip_im, [out_directory filesep ...
              sprintf('nnf_flip_im%02d.png', j) ]);      
    %% end
    end
...
%% draw the angle
function [img_angle, img_flip] = drawAngleArrow(nnf)
    angleStep = 6;
    [w h] = size(nnf(:,:,1));
    img_angle = zeros(w, h, 3);
    img_flip = zeros(w, h, 3);
    img_flip(:,:,3) = -1;
    for x = 1:w
        for y = 1:h
            index = nnf(y, x, 1);
            if index > angleStep
                img_angle(y, x, 1) = (index - angleStep)*255/angleStep;
            else
                img_angle(y, x, 2) = -(index - angleStep)*255/angleStep;
            end
            img_flip(y, x, 1) = -nnf(y, x, 2);
            img_flip(y, x, 2) = -nnf(y, x, 3);
        end
    end
    img_angle = mat2gray(img_angle);
    img_flip = mat2gray(img_flip);
end