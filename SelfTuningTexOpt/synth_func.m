...
%
    ...
    %% Global parameters
    global out_directory; %directory pathname for saving intermediate files
    global out_pathname;
    if isempty(out_directory)
        [~, out_pathname, ~] = fileparts(file);
        if isempty(out_dir)
            out_directory = out_pathname;
        else
            out_directory = [out_dir filesep out_pathname];
        end
        mkdir(out_directory);
...
%    src_gc_hist = imhist(src_gc);
%    trg_gc = histeq(trg_gc, src_gc_hist);
%    imwrite(trg_gc, [out_directory filesep 'trg_gc_histeq.png']);
    ...
    if isfield(options, 'target')
        ...
        if isfield(options, 'size')
            ...
            trg_im = create_target(size(trg_lab));
        end
    end
    ...
    %% change the src_im size
    src_im = src_im(1:size(src_lab,1), 1:size(src_lab,2), :);
    
    src_im = cat(3, src_im, src_lab);
    trg_im = cat(3, trg_im, trg_lab);
    
    load([out_directory '\src_u.txt']);
    load([out_directory '\src_v.txt']);
    load([out_directory '\trg_u.txt']);
    load([out_directory '\trg_v.txt']);
    src_u = src_u(1:size(src_lab,1), 1:size(src_lab,2));
    src_v = src_v(1:size(src_lab,1), 1:size(src_lab,2));
    src_im = cat(3, src_im, src_u);
    src_im = cat(3, src_im, src_v);
    trg_im = cat(3, trg_im, trg_u);
    trg_im = cat(3, trg_im, trg_v);
...
%% Synthesis
    ...
    while ~feof(fp)     
        ...
        out_directory = [out_dir filesep out_pathname filesep outPath];
...
function [ out] = rgb2label(image)
    image = round(255*mat2gray(image));
    out = single(image);
end
